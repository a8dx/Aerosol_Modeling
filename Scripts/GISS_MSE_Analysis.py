...
* Make title for decLinePlot graphs -- 
* muliContour should include experiment in title 
* Limit x extent to calLength value 
...
def multiContourPlot(location, pr_var, mse_var, ncon, scen, calLength):
 ...
 # ax2.set_title(r'MSE (  $\circ$ K)')
  ...
  fn = os.path.join()
  fig.savefig('fn.eps', dpi = 1200)
  #plt.tight_layout()
  plt.subplots_adjust(top=0.85)
  plt.show()

# -- keeping this to re-run the exercise easier. 
multiContourPlot("nagpur", sitename_pr, sitename_mse, 25, "1%", 360)
...
abrupt4x_path = os.path.join(basePath, "abrupt4xCO2:E2-H_abrupt4xCO2_r1i1p3_day")
rcp85H_path = os.path.join(basePath, "rcp85:E2-H_rcp85_r2i1p1_day")

hist_E2H_path = os.path.join(basePath, "E2-H_historical_r6i1p1_day")
hist_E2R_path = os.path.join(basePath, "E2-R_historical_r6i1p1_day")



# -- for now, manually read in path 
"""
os.chdir(abrupt4x_path)
scenario = [abrupt4x_path]

os.chdir(rcp85H_path)
scenario = [rcp85H_path]
...
os.chdir(hist_E2H_path)
scenario = [hist_E2H_path]
scen = "Historical (E2-H)"
...
# -- now run the analyses specified in 'cdoClass' for each variable  
...
"""
 ...
 NEED TO REMAP MULTIPLE PIXELS INTO SINGLE LOCATION --
north and east for Mumbai?  

Plot the pixels for each location?  Basemap here... 
...
"""
Hadley model output - across 1%, 4x, and multiple RCP 8.5 runs
Next stage would be to take ensemble average of the RCP 8.5 runs 
...
location_dict = {'mumbai':mumbai_loc, 'nagpur':nagpur_loc}
hadexp_dict = {'1pct':[had_1pct, "1% CO2 (HadGEM2)"], '4x':[had_4x, "Abrupt 4x CO2 (HadGEM2)"]}
...
def mse_PlottingAnalysis(path, location, scenario, calLength):
  ...
  for site in location.keys(): 
    ...
    multiContourPlot(sitename, sitename_pr, sitename_mse, 15, str(scenario), 360)
...
# -- plot for each of the Hadley experiments 
for exp in hadexp_dict.keys():
  mse_PlottingAnalysis(exp_dict[exp][0], location_dict, exp_dict[exp][1], 360)
...
"""
Produce line plots which are decadal averages for the various experiments
