...
def monthlyPlot(object, var):
  """
  Code borrowed from: http://slendermeans.org/ml4h-ch1-p5.html
  """
  nrow = 3
  ncol = 4
  fig, axes = plt.subplot(nrow, ncol, sharey = True,
  figsize = (9,11))
  plt.subplot_adjust(wspace = .05, hspace = .05)

  monthNum = 0      # month counter 
  for i in range(nrow):
    for j in range(ncol):
      xs = axes[i,j]
...
def ensemblePlot():
  ...
  for member in ensembleTotal:
    y = cdo.ymonmean(input = "-fldavg " + member, options = '-f nc')
    df = Dataset(y, mode = 'r')  
